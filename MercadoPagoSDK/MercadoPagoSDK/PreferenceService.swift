        ...
        self.request(uri: ServicePreference.MP_PREFERENCE_URI + preferenceId, params: params, body: nil, method: "GET", success: { (jsonResult) in
            if let preferenceDic = jsonResult as? NSDictionary {
                if preferenceDic["error"] != nil {
                    failure(NSError(domain: "mercadopago.sdk.PreferenceService.getPreference", code: MercadoPago.ERROR_API_CODE, userInfo: [NSLocalizedDescriptionKey: LocalizableStringsUtil.ERROR, NSLocalizedFailureReasonErrorKey: "No se ha podido obtener la preferencia".localized]))
                } else {
